  ...
  return (
    <main className="min-h-screen px-6 py-16 max-w-7xl mx-auto">
      ...
      <div className="mt-6 flex items-center gap-3">
        <button onClick={handleBuy} className="px-6 py-3 rounded-2xl bg-green-700 text-white">Buy</button>
        <select value={qty} onChange={(e) => setQty(Number(e.target.value))}
          className="border rounded-xl px-2 py-1">
          {[1,2,3,4].map(n => <option key={n} value={n}>{n}</option>)}
        </select>
        <Link href="/products" className="ml-4 px-6 py-3 rounded-2xl border">Back</Link>
      </div>
    </main>
  );
}
...
export const getStaticPaths: GetStaticPaths = async () => ({
  paths: products.map((p) => ({ params: { slug: p.slug } })), fallback: "blocking",
});
